...
import { dbService, Talk, TalkFilters } from "./database.js";
...
function formatTalks(talks: Talk[]): string {
  if (talks.length === 0) {
...
server.registerTool(
  ...
  async (params) => {
    try {
      ...
      if (params.speaker_name) filters.speaker_name = params.speaker_name;
      if (params.level) filters.level = params.level;
      if (params.track) filters.track = params.track;
      if (params.duration) filters.duration = params.duration;
      if (params.min_duration) filters.min_duration = params.min_duration;
      if (params.max_duration) filters.max_duration = params.max_duration;

      const talks = await dbService.getTalksWithFilters(filters);
      
      // Create filter summary
      const appliedFilters = [];
      if (params.speaker_name) appliedFilters.push(`Speaker: "${params.speaker_name}"`);
      if (params.level) appliedFilters.push(`Level: ${params.level}`);
      if (params.track) appliedFilters.push(`Track: "${params.track}"`);
      if (params.duration) appliedFilters.push(`Duration: ${params.duration} minutes`);
      if (params.min_duration) appliedFilters.push(`Min Duration: ${params.min_duration} minutes`);
      if (params.max_duration) appliedFilters.push(`Max Duration: ${params.max_duration} minutes`);
