...
# Initialize the active talks gauge at startup
def initialize_metrics():
    """Initialize metrics with current database state"""
    try:
        db = SessionLocal()
        current_count = db.query(models.Talk).count()
        active_talks_gauge.set(current_count)
        db.close()
        print(f"Initialized active_talks_gauge to {current_count}")
    except Exception as e:
        print(f"Failed to initialize metrics: {e}")

# Initialize metrics after database setup
initialize_metrics()
...
def health_check(db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Update the active talks gauge to keep it accurate
        current_count = db.query(models.Talk).count()
        active_talks_gauge.set(current_count)
...
def get_talks(db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Update gauge with current count (using len is fine here since we just queried all)
...
def get_talk(talk_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Update metrics (no need to update gauge since count doesn't change)
        talks_retrieved_total.inc()
...
def refresh_metrics(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "active_talks_count": current_count,
            "timestamp": time.time()
        }
    except Exception as e:
